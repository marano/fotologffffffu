...
def mock_requests_fixture_for_fotolog name
  ...
  (2003..2013).to_a.each do |year|
    (1..12).to_a.each do |month|
      stub_request(:get, "http://www.fotolog.com/#{name}/archive/#{month}/#{year}/").to_return(:body => fixture_file('photos.html'))
    end
  end

  stub_request(:get, "http://www.fotolog.com/#{name}/archive/9/2008/").to_return(:body => fixture_file('photos.html'))
end
